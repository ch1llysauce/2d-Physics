  ...
  function downloadReplayAsVideo() {
    ...
    function drawNextFrame() {
      ...
      const nextFrame = recordedFrames[index++
      ...
      const delay = nextFrame ? nextFrame.time - frame.time : 1000 / 60;
      // force visible drawing for debug
      ctx.fillStyle = "red";
      ctx.fillText("Frame " + index, 20, 20);

      setTimeout(() => requestAnimationFrame(drawNextFrame), delay);
    }
  ...
  function drawAllObjects() {
    ...
    switch (currentLesson) {
      case "freefall":
        objects.forEach(obj => {
          drawObject(ctx, obj, PixelPerMeter, RulerStartX);
      ...
      case "workEnergy":
        objects.forEach(obj => {
          drawWorkEnergyObject(ctx, obj, PixelPerMeter, RulerStartX);
          //drawVelocityArrow(ctx, obj, PixelPerMeter, RulerStartX);
        });
        break;
      default:
